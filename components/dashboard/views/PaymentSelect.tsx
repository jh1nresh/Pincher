    ...
    try {
      ...
      await supabase
        ...
        .update({ 
          ...
          payment_status: selectedMethod === 'usdc' ? 'pending_escrow' : 'pending_p2p'
        })
        .eq('trip_id', tripId)
        .eq('user_id', user.id);
    ...
    <div className="flex-1 flex flex-col p-6 h-full page-transition">
      <div className="w-full max-w-xl mx-auto space-y-6">
        ...
        <div className="space-y-3">
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-slate-400 mt-0.5">Instant • Free • No crypto needed</p>
              </div>
              {selectedMethod === 'venmo' && (
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-lg font-black text-white">USDC Escrow</span>
                  <span className="px-2 py-0.5 bg-white/10 text-slate-400 text-[9px] font-black uppercase rounded-full">
                    Premium
                  </span>
                </div>
                <p className="text-xs text-slate-400 mt-0.5">On-chain • Trustless • Auto-release</p>
              </div>
              {selectedMethod === 'usdc' && (
            ...
            {selectedMethod === 'usdc' && (
              <div className="mt-4 pt-4 border-t border-white/10 space-y-2">
                <div className="flex items-start gap-2">
                  <span className="material-symbols-outlined text-action-green text-sm">verified</span>
                  <p className="text-xs text-slate-400">Funds held in smart contract until ride completes</p>
                </div>
                <div className="flex items-start gap-2">
                  <span className="material-symbols-outlined text-action-green text-sm">shield</span>
                  <p className="text-xs text-slate-400">Auto-refund if ride is cancelled</p>
                </div>
              </div>
            )}
          </button>
        ...
        {showHandleInput && (selectedMethod === 'venmo' || selectedMethod === 'zelle') && (
          <div className="bg-white/5 border border-white/10 rounded-2xl p-5 space-y-3">
            ...
            <input
              ...
              onChange={(e) => selectedMethod === 'venmo' 
        ...
        <div className="space-y-3 pt-4">
          ...
          >
            {isSubmitting ? 'Processing...' : selectedMethod === 'usdc' ? 'Connect Wallet & Pay' : 'I\'ll Pay Now'}
          </button>
