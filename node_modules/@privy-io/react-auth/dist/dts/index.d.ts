...
import { P as PrivyEvents } from './events-context-BTan6II8.js';
export { C as CallbackError } from './events-context-BTan6II8.js';
...
type CreateSignerRequestDataType = BaseProxyRequest & {
    /** Optional: the user-specified recovery password */
...
declare const getPublicClient: (chainId: number, chains: Chain[], rpcConfig: RpcConfig, options: {
    ...
    fillTransaction: <chainOverride extends viem.Chain | undefined = undefined, accountOverride extends viem.Account | Address | undefined = undefined>(args: viem.FillTransactionParameters<Chain, viem.Account | undefined, chainOverride, accountOverride>) => Promise<viem.FillTransactionReturnType<Chain, chainOverride>>;
...
/**
 * Provider-agnostic captcha props that work across both Turnstile and hCaptcha.
 *
 * Note: Some callbacks may not be supported by all providers:
 * - `onUnsupported`: Only supported by Turnstile (browser compatibility check)
 */
type CaptchaProps = Partial<Pick<TurnstileProps, 'onUnsupported' | 'onError' | 'onSuccess' | 'onExpire'>> & {
...
declare class ConnectorManager extends EventEmitter<ConnectorManagerEvents> {
     ...
     */
    findWalletConnector(connectorType: ConnectorType, walletClientType: string, address?: string): EthereumWalletConnector | null;
    /**
...
interface PrivyInterface {
     ...
     * @param options.description {React.ReactNode} (optional) custom description for the connect wallet modal
     */
     ...
     * This will open the Privy Modal which will guide the user through this action.
     * @param options.description {React.ReactNode} (optional) custom description for the link wallet modal
...
interface ResponseTotpMfaMethod {
    type: 'totp';
    verified_at: number;
}
interface ResponsePasskeyMfaMethod {
    type: 'passkey';
    verified_at: number;
}
type LinkedMfaMethodsResponseType = Array<ResponseSmsMfaMethod | ResponseTotpMfaMethod | ResponsePasskeyMfaMethod>;
...
declare class PrivyClient {
     ...
     */
    initializeConnectorManager({ walletConnectCloudProjectId, rpcConfig, chains, defaultChain, store, walletList, shouldEnforceDefaultChainOnConnect, externalWalletConfig, appName, walletChainType, setBaseAccountSdk, }: {
...
/**
 * The interface for the `useUpdateEmail` hook.
 *
 * @param state - The current state of the email OTP process.
 * @param sendCode - Send a code to the new email address.
 * @param verifyCode - Verify the code and update the email address.
 */
...
/**
 * Use this hook to headlessly update a user's email address and to attach callbacks
 * for successful `updateEmail`, and`updateEmail` errors.
 *
 * @param callbacks.onSuccess - Callback to execute after successful email update.
 * @param callbacks.onError - Callback to execute if there is an error during email update.
 * @returns state - The current state of the email OTP process.
 * @returns sendCode - Send a code to the new email address.
 * @returns verifyCode - Verify the code and update the email address.
 */
declare const useUpdateEmail: (callbacks?: PrivyEvents["update"]) => UseUpdateEmailInterface;
...
interface UseFundWalletInterface {
     ...
     * @param options {@link FundWalletConfig} Funding configuration to specify chain and funding amount (if enabled)
     * @returns Promise that resolves when funding completes with FundingResult data
     */
...
declare const useActiveWallet: () => {
    ...
    wallet: BaseConnectedSolanaWallet | ConnectedWallet | undefined;
...
interface UseSessionSignersInterface {
    /**
     * @deprecated in favor of addSigners
    ...
    /**
     * @deprecated in favor of removeSigners
...
/**
 * Hook to add signers to a user wallet.
 *
 */
interface UseSignersInterface {
    /**
     * Adds signers to a wallet.
     *
     * @param address address of the wallet to add signers to
     * @param signers array of signers to add to the wallet
     * @returns Promise that resolves if adding signers was successful, and errors otherwise
     */
    ...
    /**
     * Removes all signers from a wallet.
     *
     * @param address address of the wallet to remove signers from
     * @returns Promise that resolves if the revocation was successful, and errors otherwise
     */
