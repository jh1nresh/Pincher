...
async function request(baseUrl, pathname, options = {}, expectedStatus = 200) {
...
async function waitUntilReady(server, baseUrl) {
  ...
  throw new Error("Next server did not become ready within 10 seconds");
}
...
const smokeSecrets = {
  webhook: "pincher-smoke-webhook",
  cron: "pincher-smoke-cron",
  mcp: "pincher-smoke-mcp",
};
const server = spawn(
  process.execPath,
  [nextBin, "start", "--hostname", "127.0.0.1", "--port", String(port)],
  {
    ...
    env: {
      ...
      TELEGRAM_BOT_TOKEN: "",
      TELEGRAM_WEBHOOK_SECRET: smokeSecrets.webhook,
      CRON_SECRET: smokeSecrets.cron,
      MCP_API_KEY: smokeSecrets.mcp,
    },
...
try {
  ...
  const homepage = await (await request(baseUrl, "/")).text();
  assert(homepage.includes("PINCHER"), "Homepage did not contain the Pincher marker");
  await request(baseUrl, "/onboarding");
  await request(baseUrl, "/trips");

  const metadata = await (await request(baseUrl, "/api/mcp")).json();
  assert(metadata.name === "pincher", "MCP metadata did not identify Pincher");
  ...
  const webhookResult = await (
    await request(baseUrl, "/api/telegram/webhook", {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-telegram-bot-api-secret-token": smokeSecrets.webhook,
      },
      body: "{}",
    })
  ).json();
  assert(webhookResult.ok === true && webhookResult.ignored === true, "Webhook smoke failed");
  await request(baseUrl, "/api/telegram/webhook?task=close-expired", {}, 401);
  ...
  const initialize = await (
    await request(baseUrl, "/api/mcp", {
      method: "POST",
      headers: {
        authorization: `Bearer ${smokeSecrets.mcp}`,
        "content-type": "application/json",
      },
      body: initializeBody,
    })
  ...
  assert(
    initialize.result?.serverInfo?.name === "pincher",
    "Authenticated MCP initialize did not return Pincher",
  );

  console.log(
    "Pincher smoke passed: live pages, MCP metadata, and fail-closed API auth boundaries.",
  );
} catch (error) {
  console.error(error);
  if (serverLog.trim()) console.error(`Next server output:\n${serverLog.trim()}`);
  process.exitCode = 1;
} finally {
