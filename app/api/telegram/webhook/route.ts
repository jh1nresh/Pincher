...
function findEventFromCommand(text: string) {
  ...
  if (!query) {
    return CONSENSUS_SIDE_EVENTS.find((event) => event.id === 'evening-with-sui-miami') || CONSENSUS_SIDE_EVENTS[0];
  }
...
function parseLeaveAt(text: string, event: ConsensusSideEvent) {
  ...
  if (!explicitTime) return getDefaultLeaveAt(event, 30);
...
function buildRideText(room: RideRoom, passengers: RidePassenger[], event?: ConsensusSideEvent) {
  ...
  const waitingList = passengers.length
    ? passengers.map((passenger) => `- ${passenger.user_name || passenger.user_id}${passenger.payment_status === 'paid' ? ' · paid' : ''}`).join('\n')
    : '- nobody yet';
...
async function editMessage(chatId: TelegramChatId, messageId: number, text: string, replyMarkup?: unknown) {
...
async function createRideRoom(chatId: TelegramChatId, user: TelegramUser | undefined, text: string) {
  const supabase = getSupabaseServerClient();
  const event = findEventFromCommand(text);
  const leaveAt = parseLeaveAt(text, event);
  const estimatedCost = estimateRideCostCents(event);
  ...
  const { data: room, error } = await supabase
    ...
    .insert({
      ...
      destination_hotzone_id: event.id,
...
async function listOpenRides(chatId: TelegramChatId) {
  ...
  const { data: rooms, error } = await supabase
    ...
    .in('status', ['open', 'full', 'splitting'])
    .in(
      'destination_hotzone_id',
      CONSENSUS_SIDE_EVENTS.map((event) => event.id),
    )
  ...
  if (error) throw error;
  if (!rooms?.length) {
    return sendMessage(chatId, `No open ride groups yet.\n\nStart one with /ride sui 6:30\nCalendar: ${CONSENSUS_CALENDAR_URL}`);
  }

  const lines = await Promise.all(
    (rooms as RideRoom[]).map(async (room, index) => {
      const passengers = await getPassengers(room.id);
      const names = passengers.map((passenger) => passenger.user_name || passenger.user_id).join(', ') || 'nobody yet';
  ...
  return sendMessage(chatId, ['Open ride groups:', '', ...lines, '', 'Use /join <id> or tap Join on a ride card.'].join('\n'));
}
...
async function handleTextMessage(message: TelegramMessage) {
  ...
  const actionMatch = text.match(/^\/(join|leave|payer|paid)(@\w+)?\s+([a-f0-9-]{6,36})(?:\s+(\S+))?/i);
